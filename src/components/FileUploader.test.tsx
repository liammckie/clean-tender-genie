...
describe('FileUploader', () => {
  ...
  it('renders the dropzone correctly', () => {
    ...
    expect(screen.getByText(/Drag & drop PDF or DOCX here/i)).toBeInTheDocument();
  });
  ...
  it('rejects unsupported file types', async () => {
    ...
    expect(mockSetError).toHaveBeenCalledWith('Only PDF or DOCX files are allowed.');
  ...
  it('uploads valid PDF file and updates state on success', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/upload_rft', expect.objectContaining({
        method: 'POST',
